...
with st.sidebar:
    ...
    with st.form(key="search_form"):
        ...
        st.slider("Number of tweets", min_value=100, max_value=1000, key="num_tweets")
...
if "df" in st.session_state:
    ...
    tab1, tab2, tab3 = st.tabs(["All", "Positif 😊", "Negatif ☹️"])
    with tab1:
        tweet_df = st.session_state.df
        make_dashboard(tweet_df, bar_color="#54A24B", wc_color="Greens")
    with tab2:
        tweet_df = st.session_state.df.query("Sentiment == 'Positif'")
        make_dashboard(tweet_df, bar_color="#1F77B4", wc_color="Blues")
    with tab3:
        tweet_df = st.session_state.df.query("Sentiment == 'Negatif'")
